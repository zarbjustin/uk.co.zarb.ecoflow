...
`stream_unit`, `stream_5000_unit` and deprecated `stream_ac5000` are physical telemetry monitors
and do not contribute to Homey Energy. `smartmeter`: `cumulative` import/export. Solar devices:
exported production. **Invariant:** cumulative meters must be **monotonic** and **single-sourced**
through the installation aggregate — flush on `onUninit` (H2) and latch off power-integration once
device counters are seen (H3).
...
Physical unit devices do not expose `measure_power` and never resolve in the backend. Preview canvases are transparent;
