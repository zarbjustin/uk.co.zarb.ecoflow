...
export function mapStreamAc5000(t: Es22Telemetry): Es22CapabilityValues {
  ...
  if (soc !== undefined) set('measure_battery', Math.max(0, Math.min(100, Math.round(soc))));
  set('battery_soh', t.bmsSohPct);
  set('measure_temperature', t.battTempC);

  // Signed battery power, positive = charging — the same convention the
  // BK-series STREAM devices use for `measure_power`.
  if (t.battW !== undefined) {
  ...
  set('measure_power.grid_import', t.gridImportPowerW);
  set('measure_power.grid_export', t.gridExportPowerW);
