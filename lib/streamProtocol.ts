...
export function streamEnvelope(sn: string, params: Record<string, any>): StreamSetEnvelope {
  return { sn, cmdId: 17, cmdFunc: 254, dirDest: 1, dirSrc: 1, dest: 2, needAck: true, params };
}
...
export const StreamCmd = {
  ...
  backupReserve: (sn: string, soc: number) =>
    streamEnvelope(sn, { cfgBackupReverseSoc: Math.max(3, Math.min(95, Math.round(soc))) }),
  feedIn: (sn: string, on: boolean) => streamEnvelope(sn, { cfgFeedGridMode: on ? 2 : 1 }),
  operatingMode: (sn: string, mode: OperatingMode) =>
    streamEnvelope(sn, { cfgEnergyStrategyOperateMode: { [MODE_PARAM[mode]]: true } }),
};
